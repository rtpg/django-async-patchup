...
DecoratorInfo = namedtuple(
    "DecoratorInfo", ["from_codegen", "unasync", "async_unsafe", "sync_variant"]
)
...
class UnasyncifyMethodCommand(VisitorBasedCodemodCommand):
    ...
    def label_as_codegen(
        self, node: FunctionDef, async_unsafe: bool, sync_variant=None
    ) -> FunctionDef:

        from_codegen_args = []
        if sync_variant is not None:
            from_codegen_args.append(
                cst.Arg(keyword=cst.Name("original"), value=sync_variant)
            )

        from_codegen_marker = Decorator(
            decorator=(cst.Call(func=Name("from_codegen"), args=from_codegen_args))
        )
        AddImportsVisitor.add_needed_import(
            self.context, "django_async_patchup.registry", "from_codegen"
        )
    ...
    def codegenned_func(self, node: FunctionDef) -> bool:
        for decorator in node.decorators:
            if m.matches(decorator.decorator, self.from_codegen_pattern):
                return True
        return False
    ...
    from_codegen_pattern = m.OneOf(
        m.Name(value="from_codegen"),
        m.Call(func=m.Name(value="from_codegen")),
    )

    def decorator_info(self, node: FunctionDef) -> DecoratorInfo:
        from_codegen = self.codegenned_func(node)
        unasync = False
        async_unsafe = False
        sync_variant = None
        ...
        if node.decorators:
            ...
            #         from_codegen = True
            if m.matches(decorator.decorator, self.generate_unasync_pattern):
                unasync = True
                args = decorator.decorator.args
                async_unsafe = False
                # Extract parameters from generate_unasynced
                for arg in args:
                    if arg.keyword and arg.keyword.value == "async_unsafe":
                        if (
                            isinstance(arg.value, cst.Name)
                            and arg.value.value == "True"
                        ):
                            async_unsafe = True
                    elif arg.keyword and arg.keyword.value == "sync_variant":
                        sync_variant = arg.value
        return DecoratorInfo(from_codegen, unasync, async_unsafe, sync_variant)
    ...
    def leave_FunctionDef(self, original_node: FunctionDef, updated_node: FunctionDef):
        ...
        if decorator_info.unasync:
            ...
            unasynced_func = self.label_as_codegen(
                unasynced_func,
                async_unsafe=decorator_info.async_unsafe,
                sync_variant=decorator_info.sync_variant,
            )
