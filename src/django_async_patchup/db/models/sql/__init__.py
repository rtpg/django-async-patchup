...
class QueryOverrides:
    ...
    @from_codegen(original=Query.get_aggregation)
    ...
    @generate_unasynced(sync_variant=Query.get_aggregation)
    ...
    @from_codegen(original=Query.get_count)
    ...
    @generate_unasynced(sync_variant=Query.get_count)
    ...
    @from_codegen(original=Query.has_results)
    def has_results(self, using):
        q = self.exists()
        compiler = q.get_compiler(using=using)
        return compiler.has_results()

    @generate_unasynced(sync_variant=Query.has_results)
    ...
    @from_codegen(original=Query.explain)
